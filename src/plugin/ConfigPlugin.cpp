...
namespace Config {
    ...
    std::string GetVersion() {
        return LOICollectionA::Expand::getInstance().getSelf().getManifest().version->to_string();
    }
    ...
    void SynchronousPluginConfigVersion(C_Config& config) {
        ...
        auto mVersion = LOICollectionA::Expand::getInstance().getSelf().getManifest().version;
